...
const items: EwasteItem[] = [
  { name: 'Teléfono móvil / Smartphone', icon: '📱', recyclable: ['Batería de litio', 'Pantalla LCD / OLED (vidrio reciclable)', 'Placa base con oro, plata, cobre', 'Carcasa de aluminio o plástico', 'Altavoces (imanes de neodimio)', 'Cámara (lentes de vidrio)'] },
  { name: 'Laptop / Notebook', icon: '💻', recyclable: ['Batería recargable', 'Disco duro o SSD (metales raros)', 'Placa madre con cobre y oro', 'Ventilador (cobre y aluminio)', 'Teclado (plástico reciclable)', 'Carcasa de aluminio o magnesio'] },
  { name: 'Monitor / Pantalla LCD', icon: '🖥️', recyclable: ['Panel LCD (vidrio)', 'Cables internos (cobre)', 'Fuente de poder (transformadores)', 'Placa controladora (oro, estaño)', 'Carcasa de plástico ABS'] },
  { name: 'Tablet / iPad', icon: '📟', recyclable: ['Batería de polímero de litio', 'Pantalla táctil (vidrio y óxido de indio)', 'Placa base (metales preciosos)', 'Carcasa de aluminio'] },
  { name: 'Televisor LED / LCD', icon: '📺', recyclable: ['Pantalla LED (vidrio)', 'Fuente de poder', 'Placa T-Con y mainboard', 'Altavoces (imanes)', 'Cables (cobre)', 'Carcasa de plástico'] },
  { name: 'Impresora', icon: '🖨️', recyclable: ['Cartuchos de tinta / tóner', 'Motor paso a paso (cobre, imanes)', 'Fuente de poder', 'Rodillos de goma', 'Placa controladora', 'Carcasa de plástico'] },
  { name: 'CPU / Torre de escritorio', icon: '🖥️', recyclable: ['Fuente de poder (cobre, aluminio)', 'Placa madre (oro, plata, cobre)', 'CPU (silicio, oro)', 'Memoria RAM (oro)', 'Disco duro (imanes, aluminio)', 'Ventiladores (cobre)'] },
  { name: 'Cargador / Fuente de poder', icon: '🔌', recyclable: ['Transformador interno (cobre)', 'Cable (cobre)', 'Placa rectificadora', 'Carcasa de plástico'] },
  { name: 'Auriculares / Audífonos', icon: '🎧', recyclable: ['Altavoces (imanes de neodimio)', 'Cable de cobre', 'Almohadillas (espuma reciclable)', 'Carcasa de plástico ABS'] },
  { name: 'Batería de litio (sueltas)', icon: '🔋', recyclable: ['Litio', 'Cobalto', 'Níquel', 'Manganeso', 'Cobre', 'Aluminio'] },
  { name: 'Cable USB / HDMI / audio', icon: '🔌', recyclable: ['Cobre (conductores internos)', 'Aislante de PVC', 'Conectores metálicos (estaño, níquel)'] },
  { name: 'Router / Módem', icon: '📡', recyclable: ['Placa base (oro, cobre)', 'Antena (cobre)', 'Fuente de poder', 'LEDs (arseniuro de galio)', 'Carcasa de plástico'] },
  { name: 'Teclado / Ratón', icon: '⌨️', recyclable: ['Placa controladora (cobre)', 'Cables (cobre)', 'Teclas de plástico ABS', 'Sensor óptico (silicio)', 'Carcasa de plástico'] },
  { name: 'Microondas', icon: '📡', recyclable: ['Transformador de alto voltaje (cobre)', 'Motor del plato giratorio', 'Ventilador (cobre)', 'Carcasa de acero', 'Placa controladora'] },
  { name: 'Lavadora / Electrodoméstico grande', icon: '🧺', recyclable: ['Motor (cobre, imanes)', 'Bomba de agua (cobre)', 'Cableado interno', 'Tambor de acero inoxidable', 'Placa controladora', 'Plásticos y gomas'] },
  { name: 'Ventilador eléctrico', icon: '🌀', recyclable: ['Motor (cobre, imanes)', 'Aspas de plástico o metal', 'Cable de poder', 'Eje de acero', 'Base de metal o plástico'] },
];
...
export default function EwastePage({ onNavigate }: Props) {
  ...
  const analyzeWithAI = async (dataUrl: string) => {
    ...
    setError(null);
    try {
      const result = await sendVisionMessage(dataUrl, 'Identifica este aparato electrónico y dime exactamente qué componentes y materiales se pueden reciclar de él. Responde en español con formato conciso: primero el nombre del aparato, luego una lista de los componentes reciclables con su material principal.');
      setAiResult(result);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen p-4 max-w-2xl mx-auto">
      ...
      <p className="text-sm text-slate-400 mb-4">O toca cada aparato para ver qué materiales se pueden recuperar y reciclar.</p>
              ...
              <div className="border-t border-slate-700/50 pt-3">
                ...
                <ul className="space-y-1">
                  {item.recyclable.map((r, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-slate-300"><span className="text-emerald-500 mt-0.5">♻️</span>{r}</li>
                  ))}
                </ul>
              </div>
            </div>
